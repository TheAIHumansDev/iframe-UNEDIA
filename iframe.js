...
<div id="claudia-launcher" onclick="openClaudia()">
  ...
  <span>💬 Saluda a UNEDIA</span>
</div>

<!-- Fullscreen Iframe -->
<div id="claudia-iframe-wrapper">
  <button id="claudia-close-btn" onclick="closeClaudia()">❌</button>
  <iframe src="https://theaihumans.com/UNEDIA-CR.html" title="UNEDIA"></iframe>
</div>
...
function openClaudia() {
  const wrapper = document.getElementById('claudia-iframe-wrapper');
  wrapper.style.display = 'block';

  const iframe = wrapper.querySelector('iframe');

  iframe.onload = () => {
    // Give it a small delay to allow script loading
    setTimeout(() => {
      iframe.contentWindow.postMessage({ action: 'activateMetahuman' }, '*');
    }, 1000);
  };
}
